...
class Llama31ToolParser(ToolParser):
    ...
    def extract_tool_calls_streaming(
        ...
        try:
            ...
            if (
                    self.tool_call_start_text not in current_text and
                    current_text not in self.tool_call_start_text
            ):
                logger.info('LLama 3 is not generating a tool call')
                return DeltaMessage(content=delta_text)

            # configure bitmask flags for partial JSON parsing
            partial_json_parsing_flags = Allow.ALL if self.current_tool_name_sent \
            ...
            if len(generated_calls) > self.current_tool_id + 1:
                ...
                logger.info(f'Set current tool id to {self.current_tool_id}')

                # if nonzero index, we need to make sure we closed the old one too
                if self.current_tool_id > 0:
                    # get the old tool call
                    old_tool_call = self.prev_tool_call_arr[
                        self.current_tool_id - 1]
                    logger.info('Checking old tool call')
                    print(f'Old tool call: {old_tool_call}')

                    # get its arguments
                    old_arguments = old_tool_call.get('arguments')
                    logger.info('Got old arguments from tool call')
                    print(f'old arguments: {old_arguments}')
            ...
            logger.info(f'Working on tool call {self.current_tool_id}: '
                        f'{working_text_for_function_call}')
            ...
            current_function_call = partial_json_parser.loads(
                ...
                partial_json_parsing_flags
            )
            logger.info(f'parsed function call {current_function_call}')
            ...
            # yet either, do that tool
                ...
                # will be None
                function_name: Optional[str] = current_function_call.get('name')
                if function_name:
            ...
            current_function_arguments = current_function_call.get('parameters')
            prev_function_arguments = self.prev_tool_call_arr[
                self.current_tool_id].get('arguments')

            logger.info(
                f'diffing old arguments {prev_function_arguments} against '
                f'current ones: {current_function_arguments}')
            if not current_function_arguments and not prev_function_arguments:
                logger.info(
                    f'Skipping text {delta_text} - no arguments are ready')

            cur_args_json = json.dumps(current_function_arguments)
            # if there are no "previous" arguments
            if not prev_function_arguments:
                logger.info(f'Finding {delta_text} in {cur_args_json}')
                arguments_delta = cur_args_json[
                                  :cur_args_json.index(delta_text) + len(
                                      delta_text)]
                logger.info(
            ...
            # if there are previous arguments and we're diffing
                ...
                logger.info(f'got argument diff, {arguments_delta}')
